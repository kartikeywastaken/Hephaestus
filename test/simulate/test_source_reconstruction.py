...
class TestSchemaVersion:
    def test_schema_version_is_5_1_0(self):
        assert SCHEMA_VERSION == "5.1.0"

    def test_artifact_schema_version(self):
        artifact = SourceReconstructionArtifact()
        assert artifact.schema_version == "5.1.0"
        d = artifact.to_dict()
        assert d["schema_version"] == "5.1.0"
...
class TestCEmitter:
    ...
    def test_basic_c_output(self):
        ...
        assert "return 0; /* placeholder */" in output
    ...
    def test_void_return(self):
        ...
        assert "return; /* placeholder */" in output
...
class TestEmitter:
    def test_write_and_read(self):
        ...
        try:
            ...
            with open(path, "r") as f:
                data = json.load(f)
            assert data["schema_version"] == "5.1.0"
            assert data["provenance"]["phase"] == "5.1"
